...
function getDefaultRegistry(userconfig) {
  if (argv.userconfig !== 'none' && fs.existsSync(argv.userconfig)) {
    ...
    var m = /^\s*(@([\w\-]+):)?registry\s*=\s*(\S+)\s*$/m.exec(content);
    if (m) {
      var scope = m[2];
      var url = m[3];
      if(scope) {
        for (var i = 0; i < argv.rawArgs.length; i++) {
          var ra = argv.rawArgs[i];
          // @scope/name or --scope=@weidai
          var rm = /^(@([\w\-]+)\/([\w\-]+))|(--scope=@([\w\-]+))$/.exec(ra);
          if(rm && (rm[2] === scope || rm[5] === scope)) {
            return url;
          }
        }
      }
